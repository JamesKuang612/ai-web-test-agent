...
import { sanitizeText, type TraceEntry } from '../shared/trace.js';
...
/** 构造 Midscene DeepSeek Vision 配置。 */
function modelConfig(): Record<string, string | number> {
  const apiKey = process.env.MIDSCENE_MODEL_API_KEY ?? process.env.DEEPSEEK_API_KEY;
  if (!apiKey) throw new Error('缺少 DEEPSEEK_API_KEY 或 MIDSCENE_MODEL_API_KEY。');
  return {
    MIDSCENE_MODEL_BASE_URL:
      process.env.MIDSCENE_MODEL_BASE_URL ?? process.env.DEEPSEEK_BASE_URL ?? 'https://api.deepseek.com',
...
/** 将 Midscene 动作事件写成现有页面可展示的轻量 Trace。 */
function appendTrace(
  trace: TraceEntry[],
  phase: string,
  data: { action?: { name?: string; target?: string }; error?: string },
): void {
...
/** 在独立可见浏览器中运行 Midscene 快速探索。 */
export class MidsceneAgent {
  async explore(instruction: string, caseId: string, stepLimit: number): Promise<MidsceneExploreResult> {
    ...
    try {
      ...
      const context = await browser.newContext({
        storageState: path.resolve('playwright', '.auth', 'jdy.json'),
      ...
      agent.addProgressListener((event) => {
        ...
        if (event.phase === 'plan_thinking') {
          console.log(`[Midscene 规划] ${data.planIndex ?? trace.length + 1}/${data.planLimit ?? '—'}`);
        }
        ...
        appendTrace(trace, event.phase, data);
      });
      output =
        (await agent.aiAct(
          `${instruction}
...
请完整执行以上测试用例，并验证其中要求的最终结果。只有在所有要求都已实际满足时才结束；无法确认时必须失败，不要猜测。`,
      ...
      if (agent) {
        ...
        reportFile = agent.reportFile ? path.relative(process.cwd(), agent.reportFile).replaceAll('\\', '/') : null;
